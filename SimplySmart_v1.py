...
class MyPanel(wx.Panel):
    ...
    def on_button2(self, event): 
        ...
        nucleus_size = [] 
        nucleus_size_calculated = [] 
        
        for file in glob.glob(self.relative_corrected_MARKER): 
            
            nucleus_size.append(file)
            
        for n_file in nucleus_size:
            
            read_file_DAPI = cv2.imread(n_file, 0)
            
            if read_file_DAPI.dtype == 'uint8':
                img_for_thresh = read_file_DAPI
            else:
                cv2.imwrite('image.jpeg', read_file_DAPI, [int(cv2.IMWRITE_JPEG_QUALITY), 100])  
                read_file_DAPI_converted = Image.open('image.jpeg')
                img_for_thresh = asarray(read_file_DAPI_converted)
            
            ret2, th2 = cv2.threshold(img_for_thresh, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
            ...
            for i in range(1, totalLabels):
                ...
                nucleus_size_calculated.append(area_pixel) 
        
        plt.hist(nucleus_size_calculated, bins=10, edgecolor='black')
        plt.title('Histogram') 
        plt.xlabel('Size [pixel]')
        plt.ylabel('Frequency')
        plt.savefig('MARKER_size.png')
        plt.close()
        
        image = cv2.imread('MARKER_size.png') 
        
        cv2.imshow(f'MARKER_size', image) 
        
    
        my_list_counts_total = []
            
        for file in glob.glob(self.relative_corrected_FOCI):
            
            test_image1 = cv2.imread(file)
            
            cv2.imwrite('image.jpeg', test_image1, [int(cv2.IMWRITE_JPEG_QUALITY), 100]) 
              
            test_image2 = Image.open('image.jpeg') 
            test_image1_array = np.array(test_image2.getdata())
            bins_hist = list(range(0,257))
            hist_image = np.histogram(test_image1_array, bins=bins_hist)
            counts1 = hist_image[0]
            counts1_to_list = counts1.tolist()
            my_list_counts_total.append(counts1_to_list)
            
                       
        summed_list = [sum(values) for values in zip(*my_list_counts_total)]
        
        mean_whole = sum(summed_list) / len(summed_list)
        
        list_shorten1=Counter(summed_list)
        list_shorten2 = [n for n in list_shorten1 if list_shorten1[n]==1]
        
        mean_shorten = sum(list_shorten2) / len(list_shorten2)
        
        final_to_recommend = mean_shorten - mean_whole
        
        my_value = min(summed_list, key=lambda x:abs(x-final_to_recommend))
        
        recommend_index = summed_list.index(my_value)
        
        plt.bar(range(len(summed_list)), summed_list, edgecolor='red', width = 0.1) 
        
        plt.title(f'mean(total) - mean(without dominance) = {recommend_index}')
        ...
        image = cv2.imread('pixel_intensity.png') 
        
        cv2.imshow(f'pixel_intensity', image) 
               
    def on_button3(self, event):
                
        try:           
            input_threshold = wx.TextEntryDialog(self, message="Provide value for threshold [from 0 to 255]")
        ...
        for s_file in sorted_list_file:
            ...
            for i in range(1, totalLabels):
                ...
                area_pixel = values[i, cv2.CC_STAT_AREA] 
                list_nucleus_area.append(area_pixel) 
        ...
        dict_param = {"threshold" : float(self.input_threshold),
    ...
    def on_button4(self, event):
        ...
        for file in glob.glob(self.relative_corrected_FOCI):
            ...
            paths_DNAdamage_raw.append(f"DNAdamage_raw/{basename}")
        ...
        for s_file in sorted_list_file: 
            ...
            if read_file_DNAdamage.dtype == 'uint8':
                img_for_thresh = read_file_DNAdamage
            else: 
                cv2.imwrite('image.jpeg', read_file_DNAdamage, [int(cv2.IMWRITE_JPEG_QUALITY), 100]) 
                read_file_DNAdamage_converted = Image.open('image.jpeg')
                img_for_thresh = asarray(read_file_DNAdamage_converted)
                                  
            ret1, th1 = cv2.threshold(img_for_thresh, threshold, 255, cv2.THRESH_TOZERO)       
            ...
            cv2.imwrite(f"DNAdamage_raw/{basename_tresh}", read_file_DNAdamage)
        ...
        for image in range(len(self.list_nucleus_all_images)): 
            ...
            for index in self.list_nucleus_all_images[image]:  
                ...
                img1 = Image.open(f"Results/Output_images_DNAdamage/DNA_damage_{name}.jpg").convert('RGBA')
                                
                img2 = Image.open(paths_DNAdamage_raw[image])  
                                  
                img1.paste(img2, (0,0), mask = img2)
                
                img1.save(f"Results/Output_images_overlaid/Overlaid_{name}.png")
        ...
        path_DNAdamage_raw = "DNAdamage_raw"
        shutil.rmtree(path_DNAdamage_raw, ignore_errors=True)
